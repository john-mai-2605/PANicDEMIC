...
def isIntensify(word): # P(IntensityMod | word) > threshold ??
    intenseList=["very", "so","really","fucking"]
    if word in intenseList:
        return True
    return False

def isDeminish(word):
    deminishList=["slightly","little"]
    if word in deminishList:
        return True
    return False
...
if __name__ == '__main__':
    ...
    for i in range(len(targetText)):
        ...
        elif ((isAdv(wTag) or isAdj(wTag)) and isDeminish(w)):
            result[i+1]=result[i]/2
    for i in range((len(targetText)//10)//20):
        print(targetText[i*10:i*10+9])
        print(result[i*10:i*10+9])
